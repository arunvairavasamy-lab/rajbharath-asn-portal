...
// Make sure the uploads/po folder exists before anything tries to save into it
...
router.post('/suppliers/add', requireAdmin, (req, res) => {
  ...
  try {
    ...
    const insertUser = db.prepare('INSERT INTO users (username, password, role, supplier_id) VALUES (?, ?, ?, ?)');
    insertUser.run(username, hashedPassword, 'supplier', newSupplierId);
    const updatedSuppliers = db.prepare('SELECT * FROM suppliers').all();
    res.render('admin-suppliers', { suppliers: updatedSuppliers, error: null, success: 'Supplier "' + name + '" added with login "' + username + '"' });
  } catch (err) {
    res.render('admin-suppliers', { suppliers, error: 'Error: ' + err.message, success: null });
  }
});

router.get('/po/upload', requireAdmin, (req, res) => {
  const suppliers = db.prepare('SELECT * FROM suppliers').all();
  const pos = db.prepare('SELECT purchase_orders.*, suppliers.name AS supplier_name FROM purchase_orders JOIN suppliers ON purchase_orders.supplier_id = suppliers.id').all();
...
router.post('/po/upload', requireAdmin, upload.single('po_file'), (req, res) => {
  ...
  try {
    ...
    const insertPO = db.prepare('INSERT INTO purchase_orders (po_number, supplier_id, po_file_path) VALUES (?, ?, ?)');
    insertPO.run(po_number, supplier_id, filePath);
    const pos = db.prepare('SELECT purchase_orders.*, suppliers.name AS supplier_name FROM purchase_orders JOIN suppliers ON purchase_orders.supplier_id = suppliers.id').all();
    res.render('admin-po-upload', { suppliers, pos, error: null, success: 'PO "' + po_number + '" uploaded successfully' });
  } catch (err) {
    const pos = db.prepare('SELECT purchase_orders.*, suppliers.name AS supplier_name FROM purchase_orders JOIN suppliers ON purchase_orders.supplier_id = suppliers.id').all();
    res.render('admin-po-upload', { suppliers, pos, error: 'Error: ' + err.message, success: null });
  }
});
